import React, {Component} from 'react';
import {Text, View, ScrollView, TouchableOpacity, Image} from 'react-native';
import {Appbar, TextInput, Card} from 'react-native-paper';
import styles from '../style/Style';
import Icon from 'react-native-vector-icons/FontAwesome';

import ChatBot from 'react-native-chatbot';

export default class Chatbot extends Component {
  state = {
    name: '',
    age: 0,
  };

  interpret = input => {
    const str = input.split(' ');
    for (var i = 0; i < str.length; i++) {
      if (str[i] == 'Army' || str[i] == 'army') {
        return 'Press to know more about the Indian Army';
      }
    }
  };

  render() {
    const steps = [
      {
        id: '1',
        message: 'Hello aspirant',
        trigger: '2',
      },
      {
        id: '2',
        message: 'What is your age?',
        trigger: '3',
      },
      {
        id: '3',
        user: true,
        inputAttributes: {
          keyboardType: 'numeric',
        },

        trigger: '4',
      },
      {
        id: '4',
        message: ({previousValue, steps}) => {
          this.setState({
            age: previousValue,
          });
          if (!isNaN(previousValue)) {
            if (previousValue >= 27)
              return (
                'Oh gosh! You fall outside the stipulated age limit.\n' +
                'But do not lose heart. There are many other ways of serving and contributing buddy!!!\n Jay Hind'
              );
            else if (previousValue < 16)
              return (
                'Contact us after ' +
                (16.5 - previousValue) +
                ' more years!\n Till then build yourself soldier!'
              );
            return `That's great!!! How can I help you?`;
          } else
            return 'First rule of the armed forces: Be honest, sincere and respectful (even with a robot)!\n Please enter a valid age!';
        },
        trigger: ({steps, value}) => {
          if (this.state.age >= 27 || this.state.age < 16) return '5';
          else if (isNaN(this.state.age)) return '3';
          else return '6';
        },
      },
      {
        id: '5',
        //  message:'All the best to you! Jay Hind!',
        component: (
          <View style={styles.cardContainer}>
            <Card.Cover
              source={{
                uri:
                  'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTZOKF9axkXGQBU_wBCYxkCfp_X7hvnYLIL9ht1t9pdox2dNpBG',
              }}
            />
          </View>
        ),
        trigger: 'Next Chance',
      },
      {
        id: 'Next Chance',
        message: 'Do you want to re-enter your age?',
        trigger: 'Next Chance Options',
      },
      {
        id: 'Next Chance Options',
        options: [
          {value: 'yes', label: 'yes', trigger: '2'},
          {
            value: 'no',
            label: 'no',
            trigger: () => {
              this.props.navigation.navigate('Home');
              this.setState({
                age: 0,
              });
              return 'enterAgain';
            },
          },
        ],
      },
      {
        id: 'enterAgain',
        message: 'Do you want to ask me something else though?',
        trigger: 'EnterAgainOptions',
      },
      {
        id: 'EnterAgainOptions',
        options: [
          {value: 'yes', label: 'yes', trigger: 'EnteredYes'},
          {
            value: 'no',
            label: 'no',
            trigger: () => {
              this.props.navigation.navigate('Home');
              this.setState({
                age: 0,
              });
              return 'enterAgain';
            },
          },
        ],
      },
      {
        id: 'EnteredYes',
        message: 'What else do you want to know?',
        trigger: '6',
      },

      {
        id: '6',
        options: [
          {value: 'Entries', label: 'Entries', trigger: '7'},
          {value: 'Services', label: 'Services', trigger: '8'},

          {
            value: ' Check Eligibility',
            label: 'Check Eligibility',
            trigger: '10',
          },
          {value: 'Apex', label: 'Know more about Apex', trigger: '9'},
          {value: 'Age', label: 'Re-enter age', trigger: '2'},
        ],
      },
      {
        id: '7',
        message: 'Check out the following entries',
        trigger: () => {
          if (this.state.age <= 19.5 && this.state.age >= 16.5) return 'NDA';
          else return 'After Grad';
        },
      },

      {
        id: 'NDA',
        options: [
          {
            value: 'NDA',
            label: 'National Defence Academy',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return '6';
            },
          },
          {
            value: 'TES',
            label: '10+2 Technical Entry Scheme',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return '6';
            },
          },
          {
            value: 'Navy TES',
            label: '10+2 Navy B.Tech',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return '6';
            },
          },
          {value: 'other', label: 'Anything else?', trigger: '6'},
        ],
      },

      {
        id: 'After Grad',
        options: [
          {
            value: 'CDS',
            label: 'Combined Defence Services',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return 'EnteredYes';
            },
          },
          {
            value: 'AFCAT',
            label: 'Air Force Common Aptitude Test',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return 'EnteredYes';
            },
          },
          {
            value: 'INET',
            label: 'Indian Navy Entrance Test',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return 'EnteredYes';
            },
          },
          {
            value: 'JAG',
            label: 'Judges and Advocate General',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return 'EnteredYes';
            },
          },
          {
            value: 'TGC',
            label: 'Technical Graduate Course',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return 'EnteredYes';
            },
          },
          {
            value: 'SSC(Tech)',
            label: 'Short Service Commission (Technical)',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return 'EnteredYes';
            },
          },
          {
            value: 'NCC',
            label: 'NCC C Certificate Special Entry',
            trigger: () => {
              this.props.navigation.navigate('Entries');
              return 'EnteredYes';
            },
          },
          {value: 'other', label: 'Anything else?', trigger: '6'},
        ],
      },
      {
        id: '8',
        message: 'Check out the following services',
        trigger: '8A',
      },

      {
        id: '8A',
        options: [
          {
            value: 'Army',
            label: 'Army',
            trigger: () => {
              this.props.navigation.navigate('AboutTheForces', {
                service: 'Indian Army',
              });
              return 'EnteredYes';
            },
          },
          {
            value: 'Navy',
            label: 'Navy',
            trigger: () => {
              this.props.navigation.navigate('AboutTheForces', {
                service: 'Indian Navy',
              });
              return 'EnteredYes';
            },
          },
          {
            value: 'Air Force',
            label: 'Air Force',
            trigger: () => {
              this.props.navigation.navigate('AboutTheForces', {
                service: 'Indian Air Force',
              });
              return 'EnteredYes';
            },
          },
          {value: 'other', label: 'Anything else?', trigger: 'EnteredYes'},
        ],
      },
      {
        id: '9',
        options: [
          {
            value: 'CDS',
            label: 'Combined Defence Services',
            trigger: () => {
              this.props.navigation.navigate('CourseInfo', {
                course: 'CDS',
              });
              return 'EnteredYes';
            },
          },
          {
            value: 'NDA',
            label: 'National Defence Academy',
            trigger: () => {
              this.props.navigation.navigate('CourseInfo', {
                course: 'NDA',
              });
              return 'EnteredYes';
            },
          },
          {
            value: 'AFCAT',
            label: 'Air Force Common Entrance Test',
            trigger: () => {
              this.props.navigation.navigate('CourseInfo', {
                course: 'AFCAT',
              });
              return 'EnteredYes';
            },
          },
          {
            value: 'INET',
            label: 'Indian Navy Entrance test',
            trigger: () => {
              this.props.navigation.navigate('CourseInfo', {
                course: 'INET',
              });
              return 'EnteredYes';
            },
          },
          {
            value: 'SPI',
            label: 'Services Preparatory Institute (Aurangabad) Entrance',
            trigger: () => {
              this.props.navigation.navigate('CourseInfo', {
                course: 'SPI',
              });
              return 'EnteredYes';
            },
          },
          {
            value: 'SSB',
            label: 'Services Selection Board',
            trigger: () => {
              this.props.navigation.navigate('CourseInfo', {
                course: 'SSB',
              });
              return 'EnteredYes';
            },
          },
          {
            value: 'Enroll',
            label: 'Contact us for more Info!',
            trigger: () => {
              this.props.navigation.navigate('Address and Location');
              return 'EnteredYes';
            },
          },
        ],
      },
      {
        id: '10',
        options: [
          {
            value: 'Army',
            label: 'Army',
            trigger: () => {
              this.props.navigation.navigate('EligibilityFormArmy');
              console.log('Pressed');
              return 'EnteredYes';
            },
          },
          {
            value: 'Navy',
            label: 'Navy',
            trigger: () => {
              this.props.navigation.navigate('EligibilityFormNavy');
              return 'EnteredYes';
            },
          },
          {
            value: 'Air Force',
            label: 'Air Force',
            trigger: () => {
              this.props.navigation.navigate('EligibilityFormAirForce');
              return 'EnteredYes';
            },
          },
          {value: 'other', label: 'Anything else?', trigger: 'EnteredYes'},
        ],
      },
    ];
    return (
      <View style={{flex: 1}}>
        <Appbar.Header>
          <Appbar.BackAction
            onPress={() => this.props.navigation.navigate('Home')}
          />
          <Appbar.Content title="Apex Chat" />
        </Appbar.Header>

        <View style={{marginBottom: '8%'}}>
          <ChatBot
            optionBubbleColor="#084d7b"
            userAvatar="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSGyZi9ahT-Z79xrSdwQvw3g3jlinQYFfCtYRFnMxeSZvWr86rI&usqp=CAU"
            botBubbleColor="#084d7b"
            botAvatar="data:image/jpeg;base64,"
            steps={steps}
          />
        </View>

        {/*  
              

             
             </ScrollView>
             <View style={{ flexDirection: 'row',
  borderBottomWidth: 1,
  borderColor: '#000',
  paddingBottom: 10,}} >
             <TextInput
    style={{flex:1}}
    autoCorrect={true}
      placeholder="Enter your message"
      onChangeText={message=>this.setState({
          message:message
      })}
     
    />
    <TouchableOpacity onPress={this.renderChat()}>
  <Icon
    name='share'
    color='#084d7b'
    size={30}
    
  />
  </TouchableOpacity>
 
 </View> */}
      </View>
    );
  }
}
